    ...
    <header className="bg-black bg-opacity-60 backdrop-blur-md border-b border-b-white border-opacity-50">
      <nav className="mx-auto flex container items-center justify-between p-6 lg:px-8" aria-label="Global">
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <a key={item.name} href={item.href} className={`text-sm font-semibold leading-6 text-white py-2 border-b border-b-white border-opacity-0 hover:border-opacity-100 transition-all duration-300 ease-in-out ${pathname === item.href ? 'active' : ''}`}>
              {item.name}
            </a>
          ))}
        </div>
              ...
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <a
                    key={item.name}
                    href={item.href}
                    className={`-mx-3 block rounded-lg px-3 py-2 text-base font-semibold leading-7 text-white hover:bg-gray-800 ${pathname === item.href ? 'active-class' : ''}`}
                  >
                    {item.name}
                  </a>
                ))}
